...
exports.read64 = function (buffer, offset, signed) {
    ...
    do {
        byte = buffer[position];
        result = result.add(long.fromNumber((byte & 0x7F) << (7 * (position - offset))));
        position++;
    } while (byte >= 0x80);
...
exports.write64 = function (buffer, number, offset, signed) {
    ...
    while (number.greaterThanOrEqual(long.fromNumber(0x80))) {
        buffer[position] = number.or(long.fromNumber(0x80)).toNumber();
